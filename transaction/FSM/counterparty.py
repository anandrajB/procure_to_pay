...
# ------------------------#
    ...
    def Draft_flow(self,request):
        ...
        self.gets_parties.status = StateChoices.NEW
        Parties.objects.filter(name = self.workflowitems.counterparty.name , city = self.workflowitems.counterparty.city).update(status = StateChoices.NEW)
        ...
        cs = workevents.objects.create(workitems=ws, from_state=StateChoices.STATUS_DRAFT, to_state=StateChoices.STATUS_DRAFT, type="COUNTERPARTY_ONBOARING", event_user = user ,comments = self.workflowitems.comments,
...
# --------------------------#
    ...
    def sent_to_bank(self,request):
        user = request.user
        ws = gets_wf_item_id(self.workflowitems.id)
        self.workflowitems.final_state = StateChoices.SENT_TO_BANK
        ...
        cs = workevents.objects.create(workitems=ws, from_state=StateChoices.SENT_TO_BANK, to_state=StateChoices.SENT_TO_BANK, type="COUNTERPARTY_ONBOARING", event_user = user ,comments = self.workflowitems.comments,
                                  interim_state=StateChoices.SENT_TO_BANK, from_party=self.workflowitems.current_from_party, to_party= self.bank)
...
# ----------------------------------#
    ...
    @stage.transition(source=stage.ANY, target=StateChoices.SENT_TO_COUNTERPARTY)
    def sent_to_counterparty(self,request):
        user = request.user
        ws = gets_wf_item_id(self.workflowitems.id)
        self.workflowitems.final_state = StateChoices.SENT_TO_COUNTERPARTY
        ...
        cs = workevents.objects.create(workitems=ws, from_state=StateChoices.SENT_TO_COUNTERPARTY, to_state=StateChoices.SENT_TO_COUNTERPARTY, type="COUNTERPARTY_ONBOARING", event_user = user ,comments = self.workflowitems.comments,
                                  interim_state=StateChoices.SENT_TO_COUNTERPARTY, from_party = self.bank , to_party= user.party)
        # to_party goes to bank
        cs.save()
...
# ----------------------#
    ...
    def reject(self,request):
        ...
        cs = workevents.objects.create(workitems=ws, from_state=StateChoices.STATUS_REJECTED, to_state=StateChoices.STATUS_REJECTED, type="COUNTERPARTY_ONBOARING", event_user = user ,comments = self.workflowitems.comments,
...
# ----------------------#
    ...
    @stage.transition(source=stage.ANY, target=StateChoices.STATUS_COMPLETED)
    def complete(self,request):
        ...
        cs = workevents.objects.create(workitems=ws, from_state=StateChoices.STATUS_COMPLETED, to_state=StateChoices.STATUS_COMPLETED, type="COUNTERPARTY_ONBOARING", event_user = user ,comments = self.workflowitems.comments,
